...
class UilOpNeoRepeatSelectUv(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        bpy.ops.view3d.nuv_interactiverectselector('INVOKE_DEFAULT', collectionIdx=bpy.context.scene.nuv_settings.last_uv_set)
        return {"FINISHED"}
...
class UtilOpNeoUvReload(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        report = nTileStorage.ImportRectData.importFile(path)
        for r in report:
            if r == "CANCELLED":
...
class UtilOpNeoUvUiNextPage(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if page * nTileUi.nuv_maxItemPerPage > items_len: page -= 1
...
class UtilOpNeoFlipUv(UtilOpMeshOperator):
    ...
    def do_mesh_edit(self, context, event, bm, in_edit_mode):
        ...
        for face in bm.faces:
            ...
            vert_len = len(face.verts)
            if vert_len == 3:
                uv_center = nMath.center_for_triangle(face.loops[0][uv_layer].uv, face.loops[1][uv_layer].uv, face.loops[2][uv_layer].uv)
            else:
                itr = 0
                for vert, loop in zip(face.verts, face.loops):
                    uv = loop[uv_layer].uv
...
class UtilOpNeoSetUvRectNormal(UtilOpMeshOperator):
    ...
    def do_mesh_edit(self, context, event, bm, in_edit_mode):
        ...
        # create dummy rect
        rect = TempNeoTileRect((-1, 1), (1, 1), (-1, -1), (1, -1))
...
class TempNeoTileRect:
    def __init__(self, topLeft, topRight, bottomLeft, bottomRight):
        ...
        # top right
        self.topRightX = topRight[0]
        self.topRightY = topRight[1]

        # bottom left
        self.bottomLeftX = bottomLeft[0]
        self.bottomLeftY = bottomLeft[1]
